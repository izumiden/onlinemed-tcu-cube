...
def carrier(gpio, frequency, micros):
   ...
   cycle = 1000.0 / frequency
   # print("cycle {} frequency {}".format(cycle, frequency))
   if 1.0 < cycle :
      ...
      for c in range(cycles):
         ...
         # print("append on  {} {} {}".format(1<<gpio, 0, on))
         wf.append(pigpio.pulse(0, 1<<gpio, off))
         # print("append off {} {} {}".format(0, 1<<gpio, off))
   else :
      # print("append {} {} {}".format(1<<gpio, 0, micros))
      wf.append(pigpio.pulse(1<<gpio, 0, micros))

   # print("wf len {}".format(len(wf)))
...
def recording(id) :
    ...
    try :
        ...
        pi.set_mode(GPIO_IR_RECV, pigpio.INPUT) # IR RX connected to this GPIO.

        pi.set_glitch_filter(GPIO_IR_RECV, GLITCH) # Ignore glitches.

        cb = pi.callback(GPIO_IR_RECV, pigpio.EITHER_EDGE, cbf)
        ...
        pi.set_glitch_filter(GPIO_IR_RECV, 0) # Cancel glitch filter.
        pi.set_watchdog(GPIO_IR_RECV, 0) # Cancel watchdog.
...
def playback(id) :
    ...
    try :
        ...
        pi.set_mode(GPIO_IR_WAVE, pigpio.OUTPUT) # IR TX connected to this GPIO.

        pi.set_mode(GPIO_IR_PORT, pigpio.OUTPUT) # IR TX connected to this GPIO.

        pi.write(GPIO_IR_PORT, pigpio.HIGH)
                ...
                for i in range(0, len(code)):
                    ...
                    if i & 1: # Space
                        ...
                        if ci not in marks_wid:
                            wf = carrier(GPIO_IR_WAVE, FREQ, ci)
        ...
        pi.write(GPIO_IR_PORT, pigpio.LOW)
    ...
    try :
        ...
        while True :
            ...
            try :
                ...
                the_same = False
                for key in ('blue', 'red', 'green', 'yellow', 'back') :

                    if key in records :

                        record = records[key]

                        the_same = compare(press, record)
                        if the_same :
                            break
...
if __name__ == '__main__':
    ...
    g.add_argument("-c", "--confirm", help="confirm keys", action="store_true")

    p.add_argument('id', nargs='+', type=str, help='IR codes')

    p.add_argument("-f", "--file", help="Filename",       type=str, default=FILE)
    p.add_argument("--freq",      help="frequency kHz",   type=float, default=FREQ)

    p.add_argument("--gap",       help="key gap ms",        type=int, default=GAP_MS)
    p.add_argument("--glitch",    help="glitch us",         type=int, default=GLITCH)
    p.add_argument("--post",      help="postamble ms",      type=int, default=POST_MS)
    p.add_argument("--pre",       help="preamble ms",       type=int, default=PRE_MS)
    p.add_argument("--short",     help="short code length", type=int, default=SHORT)
    p.add_argument("--tolerance", help="tolerance percent", type=int, default=TOLERANCE)
